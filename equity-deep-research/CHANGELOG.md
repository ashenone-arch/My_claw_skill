...
- **v2.13**：修正适用范围描述，从"A 股上市公司"扩展为"A 股/港股/美股等全球上市公司"。明确 iFinD MCP 已覆盖全球上市公司结构化数据（非仅限 A 股）。更新 SKILL.md 概述和 description 字段。
- **v2.12**：段2 新增预测变动追踪（上期预测 vs 本期预测 vs 实际）；段4 新增核心假设敏感性矩阵（前5大敏感变量、偏离幅度、净利润影响、敏感度排名）；段7 安全边际分位新增趋势方向判断（持续上行/下行/低位震荡/高位钝化）；段7 新增估值偏差归因子节（PE溢价分解为增速差异+ROE差异+不可解释残差）。方法论借鉴 CICC 投研工具 Skill 的敏感性分析、偏差归因和历史分位趋势框架。
