...
2. **主 agent 直接撰写文章**：基于并行读取到的全部内容，使用 `write` 工具直接撰写 Markdown 文章
...
保留最终 .md 文件，删除所有 .vtt、transcript_*.txt、parse_*.py 等中间文件。
...
| 错误信息 | 原因 | 解决方案 |
|----------|------|----------|
| `"This video is not available"` | 默认 web client 被限制 | 切换 android/ios client |
| `"No supported JavaScript runtime"` | 缺少 JS 运行时 | 警告可忽略；安装 deno 或 node |
| `"GVS PO Token was not provided"` | Android client 缺少 PO Token | **警告可忽略** |
...
| `TypeError: not subscriptable` | 用 dict 方式访问 dataclass | 用属性访问：`s.text` |
...
1. **永不死磕单一路径**：每个外部依赖都有独立的失败模式
2. **oEmbed API 是元信息的银弹**：标题获取 100% 可靠
3. **youtube_transcript_api 是字幕的银弹**：不受 yt-dlp 的 YouTube 反爬限制影响
4. **Android client 比 web client 更宽松**：移动端反爬限制通常更弱

## 注意事项

- **字幕路径选择**：优先 `--write-sub`（手动字幕），失败则 `--write-auto-sub`（自动字幕），再失败则 `youtube_transcript_api`
- 节目中的广告插入段落可按结构性开场白/过渡语方式处理
- **文章撰写标准**：(1) 保真底线。(2) 时间线忠实。(3) 主持人话语处理。(4) 叙事性 vs 说明性双轨。(5) 极简过渡句。(6) 专有名词标注
- 工作目录：`D:\AlphaClaw`
- Python 路径：`D:\AlphaEngine\resources\python\python\python.exe`
